...
urlpatterns = [
    ...
    url('^get_pay_token_api/(?P<pay_type>1|2)/?$', order.get_pay_token_api, {'need_login': True}),  # 订单支付页面
    ...
    url('^build_currency_order_api/(?P<currency>10|50|100|200|300|500)/(?P<istype>1|2)/?$', currency.build_curency_order_api, {'need_login': True}),  # 生成货币充值记录的api
